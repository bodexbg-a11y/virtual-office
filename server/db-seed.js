...
try {
  ...
  const insertProduct = db.raw.prepare(`
    INSERT OR IGNORE INTO products (sku, name, name_bg, category, description_bg, min_order_kg)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertAgent = db.raw.prepare(`
    INSERT OR IGNORE INTO agents (name, role, avatar_emoji, color, status, current_task)
    VALUES (?, ?, ?, ?, ?, ?)
  `);

  insertAgent.run('Стефан Димитров', 'FB Ads Manager', '📢', '#1877f2', 'online', 'Оптимизация кампании HydroBloc PU 500');
  insertAgent.run('Ивана Петрова', 'Lead Manager / CRM', '📋', '#f59e0b', 'online', 'Обработка на нови запитвания');
  insertAgent.run('Николай Георгиев', 'Content & SEO', '✍️', '#8b5cf6', 'busy', 'SEO статия за инжекционна хидроизолация');
  insertAgent.run('BodexBot AI', 'Чатбот', '🤖', '#10b981', 'online', 'Авто-отговори на клиенти 24/7');
  insertAgent.run('Мария Тодорова', 'Аналитик', '📊', '#ec4899', 'online', 'Месечен отчёт ROI');

  // CRM leads are not seeded. Real leads come from Facebook Lead Forms or manual entry.

  // Daily stats are generated from real CRM, Google Sheets and Facebook data.

  // Sample FB campaigns (demo data)
  const insertCampaign = db.raw.prepare(`
    INSERT OR IGNORE INTO fb_campaigns (fb_campaign_id, name, status, objective, daily_budget, impressions, clicks, ctr, cpc, spend, leads_count, cost_per_lead)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);

  insertCampaign.run('camp_001', 'HydroBloc PU 500 — Строителни фирми', 'ACTIVE', 'LEAD_GENERATION', 50, 12400, 342, 2.76, 0.43, 147.06, 8, 18.38);
  insertCampaign.run('camp_002', 'Ремонт на течове — Ретаргетинг', 'ACTIVE', 'LEAD_GENERATION', 30, 8200, 215, 2.62, 0.52, 111.80, 5, 22.36);
  insertCampaign.run('camp_003', 'ARCAN Пакери — Проектанти', 'PAUSED', 'TRAFFIC', 25, 5600, 178, 3.18, 0.38, 67.64, 3, 22.55);
  insertCampaign.run('camp_004', 'Хидроизолация Тунели — B2B', 'ACTIVE', 'LEAD_GENERATION', 40, 9800, 287, 2.93, 0.47, 134.89, 6, 22.48);
