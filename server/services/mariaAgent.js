...
function analyzeCampaign(campaign) {
  ...
  if (!active && leads > 0 && cpl > 0 && cpl <= 8) {
    ...
    verdict = 'Работает';
    recommendation = 'Продолжать и проверить качество лидов у Manager: дозвон, интерес, оферта.';
  }
...
function campaignPlaybook(row) {
  ...
  const common = {
    ...
    launch_plan: 'Запускать тестом 24-48 часов, бюджет ограничить, после первых лидов Manager проверяет качество.',
  };
  ...
  if (name.includes('material')) {
    return {
      ...
      launch_plan: 'Золотой запуск: включить Materials первой с небольшим бюджетом, смотреть не только CPL, а долю качественных B2B лидов после звонков Manager.',
    };
  }
  ...
  if (name.includes('услуг') || name.includes('service')) {
    return {
      ...
      conversion_recommendation: 'Форма: тип объекта, проблема, фото/описание, город, срочность. Так лиды будут понятнее для Manager.',
...
function buildOverview(rows, options = {}) {
  ...
  if (leads > 0) {
    nextActions.push('Передать все новые FB лиды Manager: звонок/Viber, статус, качество лида и причина отказа/интереса.');
  }
  ...
  return {
    ...
    golden_recommendation: best
      ? activeOnly
        ? `Золотая рекомендация Maria: по активным кампаниям главный фокус “${best.name}”. Если качество лидов у Manager подтверждается, продолжать; если лиды мусорные — не масштабировать, а менять форму и аудиторию.`
