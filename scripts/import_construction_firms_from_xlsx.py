...
def build_notes(item: dict[str, str]) -> str:
    parts = [
        ...
        f"Роль: {item.get('Роль', '').strip()}" if item.get("Роль", "").strip() else "",
...
def transform(item: dict[str, str]) -> dict[str, str]:
    ...
    phone, email = split_contact(raw_contact)
    return {
...
def main() -> int:
    if len(sys.argv) < 2:
      print("Usage: import_construction_firms_from_xlsx.py <xlsx-path>", file=sys.stderr)
      return 1
